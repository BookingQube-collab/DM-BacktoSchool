...
export const FutureIdCard = forwardRef<HTMLDivElement, Props>(function FutureIdCard(
  { profession, imageUrl, mallLogoUrl },
  ref,
) {
    ...
    >
      ...
      <div className="print-card-face flex aspect-[148/100] flex-col">
        <div className="relative flex shrink-0 items-center justify-between gap-3 bg-gradient-to-r from-primary via-primary to-accent px-5 py-3 text-white">
          ...
          <span className="shrink-0 font-display text-lg font-black tracking-wider">E3</span>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[1.15fr_1fr] gap-0">
          <div className="relative min-h-0 overflow-hidden bg-slate-100">
            ...
            />
            {mallLogoUrl ? (
              <div className="absolute bottom-2 left-2 rounded-md bg-white/15 px-2 py-1.5 backdrop-blur-[2px] print:bottom-2 print:left-2">
                <img
                  src={mallLogoUrl}
                  alt="Doha Mall"
                  className="h-9 max-w-[7.25rem] bg-transparent object-contain object-left md:h-10 md:max-w-[8.5rem] print:h-11 print:max-w-[9rem]"
                  crossOrigin="anonymous"
                />
              </div>
            ) : null}
          ...
          <div className="flex min-h-0 flex-col items-center px-3 py-3 md:px-4 md:py-4">
            ...
            <div className="flex min-h-0 w-full flex-1 flex-col items-center justify-center py-2">
              <div className="rounded-md bg-white p-1" aria-label={t("futureIdScan")}>
                <QRCodeSVG
                  value={imageUrl}
                  size={168}
                  level="M"
